...
//    );
    ...
    public static void main(String[] args) {
        ...
        var s3 = new Shawarma("Шаурма с шашлыком", new ArrayList<>(
                List.of(pita1, kebab1, cucumber2, cucumber1, tomato1, tomato2, cheese1, cheese4, souse1, souse2, souse4)));
        var s4 = new Shawarma("Шаурма Free", new ArrayList<>(
                List.of(pita1, kebab1, cucumber2, cucumber1, tomato1, tomato2, potato1, cheese1, cheese4, souse1, souse2, souse4)));
        var s5 = new Shawarma("Шаурма Fuck My Ass", new ArrayList<>(
                List.of(pita2, kebab1, chicken2, meat1, cucumber2, cucumber1, tomato1, tomato2, potato1, cheese1, cheese2, cheese3, cheese4, souse1, souse2, souse3, souse4, mushrooms1, mushrooms2, mushrooms3, potato1, potato2))
        );

        var menu = new Menu(
                Map.of(s1, 250d, s2, 300d, s3, 350d, s4, 450d, s5, 980d),
                Map.ofEntries(Map.entry(chicken1, 30d), Map.entry(chicken2, 25d), Map.entry(meat1, 33.5), Map.entry(kebab1, 37.75), Map.entry(cucumber1, 12.2), Map.entry(cucumber2, 12.3), Map.entry(tomato1, 13.7), Map.entry(tomato2, 13.8), Map.entry(cheese1, 15.2), Map.entry(cheese2, 15.4), Map.entry(cheese3, 15.6), Map.entry(cheese4, 15.8), Map.entry(souse1, 16.2), Map.entry(souse2, 16.4), Map.entry(souse3, 16.6), Map.entry(souse4, 16.8), Map.entry(mushrooms1, 17.2), Map.entry(mushrooms2, 17.4), Map.entry(mushrooms3, 17.6), Map.entry(potato1, 18.2), Map.entry(potato2, 18.4))
        );
        ...
        Order order = new Order()
                ...
                .addPosition(new OrderPosition().setQuantity(2).setShawarma(s2).setAdditionalIngredients(Map.of(cheese2, 3)))
                .addPosition(new OrderPosition().setQuantity(2).setShawarma(s2).setExcludedIngredients(List.of(cheese3)))
                .addPosition(new OrderPosition().setQuantity(2).setShawarma(s2).setAdditionalIngredients(Map.of(cheese2, 2)).setExcludedIngredients(List.of(cheese3)));
